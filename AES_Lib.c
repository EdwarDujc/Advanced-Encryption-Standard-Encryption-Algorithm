...
#define Nk (4)          //column count for a key
...
// Xtimes2[Sbox[]] Xtime 0x02 & s-box convert results table
...
// Xtimes3[Sbox[]] Xtime 0x09 & s-box convert results table
...
/*F*****************************************************************************
* PARAMS:   state       4*4 state table pointer
*
* RETURN:   None
*-------------------------------------------------------------------------------
* PURPOSE:  S-box convert and row convert, encryption
*-------------------------------------------------------------------------------
* EXAMPLE:
*-------------------------------------------------------------------------------
* NOTE:     row0 - stay same                row1 - shift left by 1 byte
*           row2 - shift left by 2 bytes    row3 - shift left by 3 bytes
*******************************************************************************/
static void ShiftRows(AES_U8 *state)
{
AES_U8 tmp;
...
* RETURN:   None
*-------------------------------------------------------------------------------
* PURPOSE:  inverse S-box convert and inverse row convert, decryption
*-------------------------------------------------------------------------------
* EXAMPLE:
*-------------------------------------------------------------------------------
* NOTE:     row0 - stay same                row1 - shift right by 1 byte
*           row2 - shift right by 2 bytes   row3 - shift right by 3 bytes
*******************************************************************************/
static void InvShiftRows(AES_U8 *state)
{
AES_U8 tmp;
...
* EXAMPLE:
*-------------------------------------------------------------------------------
* NOTE:     see each column as a polynominla on GF(2^8) multiply a fixed
*			polunominal c(x), and then mod (x^4+1),c(x)='03'x^3+'01'x^2+'01'x+'02'
*******************************************************************************/
static void MixSubColumns(AES_U8 *state)
{
AES_U8 newstate[4 * Nc];
...
* EXAMPLE:
*-------------------------------------------------------------------------------
* NOTE:     used in both encryption and decryption
*******************************************************************************/
static void AddRoundKey(AES_U8 *state, AES_U8 *key)
{
AES_U16 idx;    //index
...
/*F*****************************************************************************
* PARAMS:   key         key table pointer
*           expKey      expandkey table pointer
* RETURN:   None
*-------------------------------------------------------------------------------
* PURPOSE:  expand key, AES initialization
*-------------------------------------------------------------------------------
* EXAMPLE:
*-------------------------------------------------------------------------------
...
AES_U16 idx;    //index
    ...
    for (idx=Nk; idx<(Nc * (Nr + 1)); idx++)  //
    {
        ...
        if ( !(idx % Nk) )
        {
            tmp4 = tmp3;
            tmp3 = Sbox[tmp0];
            tmp0 = Sbox[tmp1] ^ Rcon[idx/Nk];	//if idx mod Nk==0, xor a constant Rcon
            tmp1 = Sbox[tmp2];
            tmp2 = Sbox[tmp4];
        } //End of if
...
/*F*****************************************************************************
* PARAMS:   in          input buffer pointer
*           expKey      expand key table pointer
*           out         output buffer pointer
* RETURN:   None
*-------------------------------------------------------------------------------
* PURPOSE:  128 bytes encryption
*-------------------------------------------------------------------------------
* EXAMPLE:
*-------------------------------------------------------------------------------
* NOTE: 
...
{
AES_U8  state[Nc * 4];  //state array
AES_U16 round;          //encryption rounds counter
AES_U16 idx;            //index
...
AES_U16 idx;            //index
    ...
    //inverse s-box convert, inverse row convert
    InvShiftRows(state);
